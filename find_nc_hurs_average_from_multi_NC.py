...
for w in fn:
    ...
    #无气压下
    var_f = nc_obj.variables[var][ind_t, min_lat:max_lat, min_lon:max_lon]  #无气压下
    # 记录上次的索引号
    last_lat= 0
    last_lon = 0
    year_l = []
    ...
    for index, row in df.iterrows():
        ...
        if last_lat!=lat_ind or last_lon!=lon_ind:
            ...
            t_var = t_var.tolist()
            #t_var = [round(i, 2) for i in t_var] #小数两位对于变量过小的不可用
            t_var = [i for i in t_var]
            last_lat=lat_ind
            last_lon=lon_ind
        year_ll += year_l
        month_ll += month_l
...
#保存结果
gf.to_csv(save_dir +'\\'+ var+'_NC_hurs_mean_SSP5.txt', sep='\t', index=False)
